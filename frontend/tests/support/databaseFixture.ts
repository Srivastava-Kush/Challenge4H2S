...
export async function openDatabaseBackedFanApp(page: Page) {
  ...
  // Mock telemetry endpoint to prevent hanging
  ...
  // Wait for the landing page button and click it
  ...
  // Wait for the main app content (SVG map) to render
  await page.locator('svg.map-svg').waitFor({ state: 'visible', timeout: 10000 });
}
